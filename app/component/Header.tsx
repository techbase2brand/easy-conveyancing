...
const Header = memo(() => {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-[#ECECEC] py-2 px-4 ${isScrolled ? 'shadow-md bg-white' : ''}`}>
      <div className="flex items-center justify-between max-w-7xl mx-auto gap-4 sm:gap-8">
        ...
        <Link 
          ...
          <Image  src="/logo.webp"  alt="Company Logo"
            width={80}
            height={1000}
        ...
        <nav className="hidden md:flex items-center gap-6 lg:gap-8 flex-1 justify-center" aria-label="Main navigation">
          {navLinks.map((link) => (
            <a  key={link.href} href={link.href} onClick={(e) => handleLinkClick(e, link.href)} className={`text-sm lg:text-base font-normal transition-all duration-200 rounded px-3 py-2 ${
                link.label === 'Home'
        ...
        {/* Desktop CTA Button */}
        <Link href="https://wa.me/61433101353" target="_blank" rel="noopener noreferrer" aria-label="Start chat"
        ...
        >
         {whatsappIcon("desktop")}
          <span className="font-normal">Chat Now</span>
        </Link>

        {/* Mobile Navigation - Dropdown with improved styling */}
        <nav id="mobile-navigation" ref={mobileMenuRef} className={`absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-xl md:hidden overflow-hidden transition-all duration-300 ease-in-out ${
            isMobileMenuOpen ? 'max-h-[calc(100vh-4rem)] opacity-100' : 'max-h-0 opacity-0'
          }`}
          aria-label="Mobile navigation" aria-hidden={!isMobileMenuOpen}>
