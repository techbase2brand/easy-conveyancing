...
const Header = memo(() => {
  ...
  const whatsappIcon = ((id: string) => (
    <svg width={22} height={22} viewBox="0 0 48 49" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              return (
                ...
                >
                  ...
                  {activeServiceSubmenu === link.label && servicesWithSubmenu.subItems && (
                    <div className="absolute top-full left-0 mt-1 w-72 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden z-50">
                      <div className="py-2">
                        {/* Submenu Items */}
                        {servicesWithSubmenu.subItems.map((subItem) => (
                          <Link
                            key={subItem.href}
                            href={subItem.href}
                            className="block px-4 py-2  transition-colors group"
                            onClick={() => setActiveServiceSubmenu(null)}>
                            <div className="text-sm font-regular text-secondary hover:text-primary hover:underline transition-colors">
                              {subItem.label}
                            </div>
