    ...
    <section className="px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      <div className="bg-primary rounded-2xl max-w-[1320px] mx-auto p-4 sm:p-8 md:p-10 lg:p-12 text-white">
        <div className="flex  sm:flex-row justify-between items-center gap-5 mb-8">
          {title && (
            <h2 className="text-[24px] md:text-[40px] font-bold text-transparent stroke-text-2 mb-4 md:mb-4 border-b border-[#D9D9D9] inline-block">
              {title}
            </h2>
          )}
        ...
        {description && (
          <p className="text-base font-semibold pb-3 md:pb-6 md:text-lg text-white">
            {description}
          </p>
        )}

        {itemHeading && (
          <h4 className="text-sm md:text-lg sm:text-xl font-medium mb-4">
            {itemHeading}
          </h4>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
            ...
            <div className="image mb-0 md:mb-[-150px] relative">
              {imageUrl && (
                <Image 
                  width={700}
                  height={1000}
